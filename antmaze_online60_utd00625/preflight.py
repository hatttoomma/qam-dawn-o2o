...
assert {k:v for k,v in manifest['files'].items() if k!='legacy/run.py'} == {k:v for k,v in old['files'].items() if k!='legacy/run.py'}
original=(ROOT.parent/'qam_antmaze/legacy/run.py').read_text()
modified=(ROOT/'legacy/run.py').read_text()
assert common.file_hash(ROOT.parent/'qam_antmaze/legacy/run.py')==manifest['parent_run_sha256']==old['files']['legacy/run.py']
assert modified.count('target_updates=2500+(step-50000)//16')==1
assert ast.dump(ast.parse(modified.replace('target_updates=2500+(step-50000)//16','target_updates=int((step-args.warmup)*.25)')))==ast.dump(ast.parse(original))
for step,expected in [(50000,2500),(50015,2500),(50016,2501),(50032,2502),(60000,3125)]:
 assert 2500+(step-50000)//16==expected
...
common.atomic_json(ROOT/'results/preflight.json',dict(status='passed',tasks=tasks,
    only_update_schedule_changed=True,all_four_full_resume_checkpoints_verified=True))
