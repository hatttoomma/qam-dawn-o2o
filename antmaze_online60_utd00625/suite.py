...
def status(**kwargs):
    completed=[task for task in (1,2,3,5) if (OUT/f'task{task}/dawn/CHECKS_PASSED.json').exists()]
    write(OUT/'suite_status.json',dict(utc=utc(),suite_pid=os.getpid(),tasks=list((1,2,3,5)),
...
def verify(folder,smoke):
    ...
    assert (done['steps'],done['updates']) == ((50032,2502) if smoke else (60000,3125))
    assert checks['evaluation_steps_verified'] == ([50032] if smoke else [60000])
