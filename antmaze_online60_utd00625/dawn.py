"""Resume exact balanced50k state; online-only continuation with UTD0.0625."""
import fcntl
import json
import os
...
TASK=int(os.environ['ANTMAZE_TASK']);assert TASK in (1,2,3,5)
...
common.GRID=(50000,50032) if SMOKE else (50000,60000)
args=SimpleNamespace(stage='warm',out=str(OUT),data='/root/.ogbench/data',seed=0,
    offline_steps=500000,online_steps=50032 if SMOKE else 60000,
    ...
    td_target='naive',actor_input='state+base_action',utd=.0625,task_id=TASK,
    environment=common.ENV,checkpoint_steps=list(common.GRID),resume_env_step=50000,
    resume_updates=2500,additional_env_steps=32 if SMOKE else 10000,
...
source_resume=next(x for x in json.loads((ROOT/'results/preflight.json').read_text())['tasks'] if x['task_id']==TASK)
...
class Factory:
    ...
    def create(qam,obs,seed,warm):
        ...
        config=dict(old,utd=args.utd,online_steps=args.online_steps,checkpoint_steps=list(common.GRID),
        ...
        reference=json.loads((ROOT.parent/f'qam_antmaze_online60/results/task{TASK}/dawn/actual_agent_config.json').read_text())
        ref_diff={k:[reference.get(k),config.get(k)] for k in set(reference)|set(config) if reference.get(k)!=config.get(k)}
        ref_allowed={'utd','checkpoint_steps','source_files'} | ({'online_steps','additional_env_steps'} if SMOKE else set())
        assert set(ref_diff)<=ref_allowed,ref_diff
        common.atomic_json(OUT/'utd_reference_comparison.json',dict(status='passed',
            reference_root=str(ROOT.parent/'qam_antmaze_online60'),changed_fields=ref_diff,
            all_other_hyperparameters_identical=True))
...
def stack(records,indices):
    ...
    additional_updates=(len(records)-50000)//16
    if additional_updates==1 or additional_updates%125==0 or len(records)==args.online_steps:
...
def save(path,agent,**payload):
    assert int(agent.updates)==2500+(payload['step']-50000)//16
...
try:
    ...
    assert done['steps']==args.online_steps and done['updates']==2500+args.additional_env_steps//16
    assert done['final_flow_hash']==offline['flow_hash']
    assert common.file_hash(OUT/'final.pkl')==done['checkpoint_sha256']
    assert audit['additional_updates']==args.additional_env_steps//16 and audit['additional_offline_draws']==0
    ...
    assert not list(OUT.glob('*055000*')), 'No55k evaluation or checkpoint in this arm'
    common.atomic_json(OUT/'CHECKS_PASSED.json',dict(status='passed',task_id=TASK,
        ...
        frozen_base_verified=True,online_only_replay_verified=True,unchanged_update_equations=True,continuation_utd=args.utd,
        steps=done['steps'],updates=done['updates'],additional_steps=args.additional_env_steps,
        additional_updates=args.additional_env_steps//16,evaluation_steps_verified=list(common.GRID[1:])))
