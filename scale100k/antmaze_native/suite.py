...
gpu=open('/tmp/qam_scale100k_gpu.lock','w')
def write(p,v):
 t=p.with_suffix('.tmp');t.write_text(json.dumps(v,indent=2));t.replace(p)
def utc():return time.strftime('%Y-%m-%dT%H:%M:%SZ',time.gmtime())
def status(**kw):
 write(OUT/'suite_status.json',dict(utc=utc(),pid=os.getpid(),completed=[t for t in range(1,6) if (OUT/f'task{t}/native/CHECKS_PASSED.json').exists()],**kw))
def run(task,smoke=False):
 folder=OUT/('smoke' if smoke else f'task{task}')/'native';folder.mkdir(parents=True,exist_ok=True)
 ...
 with (folder/'stdout.log').open('w') as log:
  p=subprocess.Popen([sys.executable,'-u',str(ROOT/'native.py')],cwd=ROOT,env=env,stdout=log,stderr=subprocess.STDOUT)
 ...
 if smoke:assert json.loads((folder/'LOGGING_CHECK_PASSED.json').read_text())['status']=='passed'
 if not smoke:assert (checks['steps'],checks['updates'])==(100000,95001)
try:
 ...
 run(1,True)
 status(state='waiting_gpu',smoke_passed=True)
