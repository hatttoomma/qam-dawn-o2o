...
def residual(args):
    if (Path(args.out)/'DONE.json').exists():
        return
    out, env, ds, ex, qam = prepare_online(args)
    mixed = BalancedReplay(ds,qam,args)
    del ds
    initial_flow = flow_hash(qam)
    ...
    while step<args.online_steps:
        ...
        if enabled:
            ...
            target_updates=max(0,min(step,args.replay_switch)-args.warmup)//4+max(0,step-args.replay_switch)//16
            n=target_updates-int(agent.updates)
            for _ in range(n):
                if int(agent.updates)<(args.replay_switch-args.warmup)//4:
                    batch=mixed.sample(replay,args.dawn_batch,int(agent.updates))
                else:
                    idx=np.random.randint(len(replay),size=args.dawn_batch)
                    batch=stack_replay(replay,idx)
