"""Additional DAWN arms: QAM replay, with the original three runs preserved."""
import argparse
import fcntl
import json
...
def residual(args):
    ...
    config.update(replay_mode='qam_uniform_offline_online_sequences', critic_boundary_mask=True,
    ...
    agent = DawnQamReplayAgent.create(qam,ex['observations'],args.seed,warm=args.stage=='warm')
    ...
    while step<args.online_steps:
        ...
        if step//1000>last_log or step==args.online_steps:
            ...
            log(out/'metrics.jsonl',dict(step=step,updates=int(agent.updates),elapsed_seconds=elapsed,replay_chunks=len(replay),**buffer.stats(),**metrics))
    ...
    assert flow_hash(qam)==initial_flow
    atomic_json(out/'DONE.json',dict(arm=args.stage+'_qam_replay',steps=step,updates=int(agent.updates),initial_hashes=initial,
...
def parse():
    ...
    p.add_argument('--stage',choices=['cache','warm','random'],required=True)
...
if __name__=='__main__':
    ...
    try:
        cache_offline(args) if args.stage=='cache' else residual(args)
