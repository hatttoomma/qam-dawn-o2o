...
parser.add_argument('--socket', default='/tmp/icra_antmaze45557.sock')
parser.add_argument('--full', action='store_true', help='Include episode records, metrics, and checkpoint verification.')
parser.add_argument('--output', default='/Users/riverwang/Documents/ICRA2026/output/antmaze_large_task1_20260913/remote_status.json')
args = parser.parse_args()
source = '''
from pathlib import Path
import json,os,subprocess,time,hashlib,gzip,base64
root=Path('/root/autodl-tmp/qam_antmaze')
...
if FULL:
 for stage in ('native','dawn'):
  for p in sorted((root/'results'/stage).glob('*.jsonl')):
 ...
 for relative,metadata,key in (
   ('results/native/offline_500k.pkl','results/native/offline_checkpoint.json','sha256'),
   ('results/native/native_final.pkl','results/native/native_checkpoint.json','sha256'),
   ('results/dawn/final.pkl','results/dawn/DONE.json','checkpoint_sha256')):
...
for p in sorted((root/'results').glob('*/stdout.log')):
 with p.open('rb') as f:
