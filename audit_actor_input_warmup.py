...
def main():
    ...
    for task in (1, 2):
        base = None
        for mode in ('masked', 'base_action'):
            name = f'task{task}_{mode}'; folder = RUNS / name
            ...
            (folder/'evaluation_state_check.json').write_text(json.dumps(dict(task=task,actor_input=mode,
            ...
            rows[name] = dict(source_checkpoint=str((folder/'latest.pkl').relative_to(ROOT)),
                ...
                snapshot_sha256=hashlib.sha256(snap.read_bytes()).hexdigest(), differences_from_masked=diffs)
            historical = ROOT / f'runs/td_ablation/batch_comparison/warmup_audit/task{task}_b256.npz'
