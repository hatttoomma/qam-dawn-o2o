...
parser.add_argument('--socket', default='/tmp/icra_antmaze45557_tasks2_5.sock')
parser.add_argument('--full', action='store_true', help='Include episode records, metrics, and checkpoint verification.')
parser.add_argument('--output', default='/Users/riverwang/Documents/ICRA2026/output/antmaze_large_tasks2_5_20260913/remote_status.json')
args = parser.parse_args()
source = '''
from pathlib import Path
import json,os,subprocess,time,hashlib,gzip,base64
root=Path('/root/autodl-tmp/qam_antmaze_task2_5')
...
if FULL:
 ...
 for task in (2,3,4,5):
  for relative,metadata,key in (
    (f'results/task{task}/native/offline_500k.pkl',f'results/task{task}/native/offline_checkpoint.json','sha256'),
    (f'results/task{task}/native/native_final.pkl',f'results/task{task}/native/native_checkpoint.json','sha256'),
