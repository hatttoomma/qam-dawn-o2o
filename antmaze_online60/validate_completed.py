...
for task in range(1,6):
    ...
    assert checks['status']=='passed' and (done['steps'],done['updates'],checks['task_id'])==(60000,5000,task)
    assert (checks['additional_steps'],checks['additional_updates'])==(10000,2500)
    assert checks['evaluation_steps_verified']==[55000,60000]
    for key in ('exact50k_agent_and_optimizers_resumed','online_replay_restored','environment_restored',
        ...
        assert checks[key]
    expected=dict(online_steps=60000,warmup=40000,batch_size=256,utd=.25,seed=0,task_id=task,
        ...
        checkpoint_steps=[50000,55000,60000])
    ...
    resume=get('resume_audit.json');pre=files['results/preflight.json']['tasks'][task-1]
    ...
    assert (audit['online_buffer_size'],audit['cumulative_updates'],audit['additional_updates'])==(60000,5000,2500)
    assert (audit['additional_online_draws'],audit['additional_offline_draws'])==(640000,0)
    ...
    for row in get('metrics.jsonl'):
        assert 50000<row['step']<=60000 and row['updates']==(row['step']-40000)//4
    assert get('metrics.jsonl')[-1]['step']==60000
    for step in (55000,60000):
    ...
    for step in (50000,55000,60000):
        for mode,suffix in [('mean','_mean_residual'),('sampled','')]:
            name=f'eval_{step:06d}_100{suffix}.json'
            method=('Parent balanced ' if step==50000 else 'Continuation online-only ')+mode
            sources.append((method,get(name),(step-40000)//4,prefix+name))
    rows=[]
    ...
    lines+=['','The continuation adds interaction and updates while changing replay; it does not isolate replay alone.']
