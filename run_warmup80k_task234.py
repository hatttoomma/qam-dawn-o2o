"""Task2/3/4 extension of the validated Task1/5 warmup-80k, online-150k runner."""
import argparse
import fcntl
import json
import pickle
...
RUNS = common.ROOT / 'runs/warmup80k_150k_task234_20260911'
OFFLINES = {2: ('runs/task2/offline/final.pkl', 'b117517f8e6d2227137a715de4c5045c0468b252949c0c782e94265dd489cd02'),
...
def main():
    ...
    extra.add_argument('--task', type=int, choices=(2, 3, 4), required=True)
    ...
    class Factory:
        ...
        def create(qam, obs, seed, warm):
            ...
            if warm:
                previous_path = ('runs/actor_input_ablation/task2_base_action' if task == 2 else f'runs/cube5/task{task}_warm')
    ...
    def save(path, agent, **payload):
        ...
        if Path(path).name == 'latest.pkl':
            ...
            expected_updates = 0 if step <= boundary else (step-args.warmup)//4
