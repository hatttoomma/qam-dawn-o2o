"""One-GPU DAWN-only queue; preserved per-task offline checkpoints."""
import fcntl
import hashlib
import json
import os
...
def verify(folder,smoke):
    ...
    assert (done['steps'],done['updates']) == ((48,2) if smoke else (50000,2500))
    assert checks['evaluation_steps_verified'] == ([44,48] if smoke else [45000,50000])
