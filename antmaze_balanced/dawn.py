"""AntMaze 40k+10k DAWN with fixed 50:50 offline/online replay."""
import fcntl
import json
import os
...
common.GRID = (0, 40, 44, 48) if SMOKE else (0, 40000, 45000, 50000)
args = SimpleNamespace(stage='warm', out=str(OUT), data='/root/.ogbench/data', seed=0,
    offline_steps=500000, online_steps=48 if SMOKE else 50000,
    offline_checkpoint=str(NATIVE / 'offline_500k.pkl'), eval_episodes=2 if SMOKE else 100,
    final_episodes=2 if SMOKE else 100, warmup=40 if SMOKE else 40000, dawn_batch=256,
    td_target='naive', actor_input='state+base_action', utd=.25, task_id=TASK,
    environment=common.ENV, checkpoint_steps=list(common.GRID),
    replay='balanced_offline_online', offline_batch_size=128, online_batch_size=128)
...
assert native_done['online_env_steps'] == 50000
assert native_done['online_updates'] == 45001
...
def load_data(config):
    ...
    assert native_manifest['task_id'] == TASK and native_manifest['environment'] == common.ENV
...
class Factory:
    ...
    def create(qam, obs, seed, warm):
        ...
        common.atomic_json(OUT / 'actual_agent_config.json', dict(
            ...
            replay='balanced_offline_online', offline_batch_size=128, online_batch_size=128,
            offline_base_actions='frozen_QAM_stochastic_resampled_per_update', critic_ensemble=10,
            ...
            frozen_base_inv_temp=10., seed=0, task_id=TASK, checkpoint_steps=list(common.GRID),
            source_files=manifest['files']))
        prior = json.loads((NATIVE.parent / 'dawn/actual_agent_config.json').read_text())
        actual = json.loads((OUT / 'actual_agent_config.json').read_text())
        allowed = {'warmup', 'online_steps', 'replay', 'checkpoint_steps', 'source_files'}
        differences = {k: [prior[k], actual.get(k)] for k in prior if prior[k] != actual.get(k)}
        assert set(differences) <= allowed, differences
        common.atomic_json(OUT / 'parameter_comparison.json', dict(status='passed',
            changed_existing_fields=differences, additional_fields={k:v for k,v in actual.items() if k not in prior},
...
def evaluate(qam, folder, step, config, **kwargs):
    ...
    if step == 0:
        native_base = json.loads((NATIVE / 'fixed_eval/eval_000000_100.json').read_text())
        assert result['records'] == native_base['records'][:args.eval_episodes], 'Base policy or paired evaluation differs'
    ...
    common.atomic_json(OUT / f'eval_rng_preserved_{step:06d}.json', dict(
        status='passed', step=step, numpy_rng_unchanged=True))
    return result
...
try:
    ...
    for step in common.GRID[2:]:
        for suffix in ('', '_mean_residual'):
            result = json.loads((OUT / f'eval_{step:06d}_{args.eval_episodes:03d}{suffix}.json').read_text())
            assert result['step'] == step and result['episodes'] == args.eval_episodes
    audit = json.loads((OUT / 'replay_audit.json').read_text())
    assert audit['update'] == done['updates']
    assert audit['offline_per_batch'] == audit['online_per_batch'] == 128
    assert audit['offline_draws'] == audit['online_draws'] == done['updates']*128
    assert audit['online_buffer_size'] == args.online_steps
    common.atomic_json(OUT / 'CHECKS_PASSED.json', dict(status='passed',
        unchanged_collection_and_update_equations=True, balanced_replay_verified=True, native_gate_passed=True,
        offline_checkpoint_matched=True, hard_TD_verified=True, frozen_base_verified=True,
        steps=done['steps'], updates=done['updates'], task_id=TASK,
        evaluation_steps_verified=list(common.GRID[2:])))
